...
class SkipDBPlayer(xbmc.Player):
    def __init__(self) -> None:
        super(SkipDBPlayer, self).__init__()
    ...
    def onAVStarted(self) -> None:
        ...
        xbmc.log('[SkipDB] Playback started: {} (tv={}, video={})'.format(
    ...
    def onPlayBackPaused(self) -> None:
        ...
        self._pause_count += 1
        xbmc.log('[SkipDB] Playback paused (count={})'.format(self._pause_count), xbmc.LOGINFO)

    def onPlayBackResumed(self) -> None:
        self._is_paused = False

    def _reset(self) -> None:
        xbmc.log('[SkipDB] Playback ended/stopped', xbmc.LOGINFO)
    ...
    def get_media_ids(self) -> Dict[str, Any]:
        ...
        # SkipDB keys TV segments on the show's IMDb id + season/episode, not the episode's own id
        if not ids.get('is_movie') and ids.get('show_imdb_id'):
            ids['lookup_imdb_id'] = ids['show_imdb_id']
        else:
            ids['lookup_imdb_id'] = ids.get('imdb_id')

        xbmc.log('[SkipDB] Extracted media IDs: {}'.format(ids), xbmc.LOGINFO)
        return ids
    ...
    def get_next_episode(self) -> Optional[Dict[str, Any]]:
        ...
        if next_episode and ADDON.getSetting('debug_logging') == 'true':
            xbmc.log(
                '[SkipDB] Next episode found: S{}E{} {}'.format(
                    next_episode['season'],
    ...
    def play_next_episode(self, next_episode: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if ADDON.getSetting('debug_logging') == 'true':
            xbmc.log('[SkipDB] Failed to open next episode: {}'.format(response), xbmc.LOGWARNING)
        return False
    ...
    def _apply_show_imdb(self, unique: Any, imdbnumber: Any, ids: Dict[str, Any]) -> None:
        if ids.get('show_imdb_id'):
            return
        imdb_unique = unique.get('imdb') if isinstance(unique, dict) else None
        for candidate in (imdb_unique, imdbnumber):
            if candidate and str(candidate).startswith('tt'):
                ids['show_imdb_id'] = str(candidate)
                if ADDON.getSetting('debug_logging') == 'true':
                    xbmc.log('[SkipDB] TV show IMDb resolved: {}'.format(candidate), xbmc.LOGINFO)
                return
    ...
    def _tvshow_ids_from_library(self, tvshowid: Any, ids: Dict[str, Any], force_tmdb: bool = False) -> None:
        ...
        if show_tmdb and (force_tmdb or not ids.get('tmdb_id')):
            ...
            if ADDON.getSetting('debug_logging') == 'true':
                xbmc.log('[SkipDB] TV show TMDB resolved: {} -> {} (uniqueid={})'.format(
                    old or '-', show_tmdb, unique), xbmc.LOGINFO)

        self._apply_show_imdb(unique, details.get('imdbnumber'), ids)
    ...
    def _tvshow_ids_by_title(self, title: Optional[str], ids: Dict[str, Any], force_tmdb: bool = False) -> None:
        ...
        if show_tmdb and (force_tmdb or not ids.get('tmdb_id')):
            ...
            ids['tmdb_id'] = show_tmdb
            if ADDON.getSetting('debug_logging') == 'true':
                xbmc.log('[SkipDB] TV show TMDB resolved (title match): {} -> {} (title={} uniqueid={})'.format(
                    old or '-', show_tmdb, title, unique), xbmc.LOGINFO)

        self._apply_show_imdb(unique, pick.get('imdbnumber'), ids)
    ...
    def _ids_from_jsonrpc(self, ids: Dict[str, Any]) -> None:
        try:
            ...
            if ADDON.getSetting('debug_logging') == 'true':
                xbmc.log('[SkipDB] JSON-RPC item: type={} uniqueid={} imdbnumber={}'.format(
            ...
            xbmc.log('[SkipDB] JSON-RPC Player.GetItem failed: {}'.format(e),
                      xbmc.LOGWARNING)
    ...
    def _ids_from_infotag(self, ids: Dict[str, Any]) -> None:
        ...
        if not ids.get('show_imdb_id'):
            try:
                for key in ('tvshow.imdb', 'imdbshow', 'imdb_show', 'tvshowimdb'):
                    show_imdb = tag.getUniqueID(key)
                    if show_imdb and str(show_imdb).startswith('tt'):
                        ids['show_imdb_id'] = str(show_imdb)
                        break
            except Exception:
                pass
