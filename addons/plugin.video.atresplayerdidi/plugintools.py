...
def read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    ...
    # Cookies instaladas, lanza la peticiÃ³n
    ...
    # Actualiza el almacÃ©n de cookies
...
def add_item( action="" , title="" , plot="" , url="" , thumbnail="" , fanart="" , show="" , episode="" , extra="", page="", info_labels = None, isPlayable = False , folder=True, ref_id="" ):
    ...
    if url.startswith("plugin://"):
        ...
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s&ref_id=%s' % ( sys.argv[ 0 ] , action , urllib.parse.quote_plus( title ) , urllib.parse.quote_plus(url) , urllib.parse.quote_plus( thumbnail ) , urllib.parse.quote_plus( plot ) , urllib.parse.quote_plus( extra ) , urllib.parse.quote_plus( page ) , urllib.parse.quote_plus( ref_id ))
...
def play_resolved_url(url, subtitles=None, headers=None, subtitle_offset=None):
    """
    Play a video URL in Kodi with optional subtitles, headers, and offset (GUI + internal)
    """
    ...
    # return OFFSETPLAYER.play(url, listitem)
    xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, listitem)
    if subtitle_offset:
        xbmc.executebuiltin(f"SetSubtitleDelay({int(subtitle_offset*1000)})")
        _log(f"Set subtitle offset: {subtitle_offset}s")
