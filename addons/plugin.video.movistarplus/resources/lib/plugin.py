...
def select_account(id, name):
  ...
  add_menu_option(addon.getLocalizedString(30453), get_url(action='import_session')) # Import session
  if m.logged:
    add_menu_option(addon.getLocalizedString(30452), get_url(action='export_session')) # Export session
  close_folder(cacheToDisc=False)
...
def router(paramstring):
  ...
  if params:
    if params['action'] == 'play':
      ...
      import_credentials()
    elif params['action'] == 'export_session':
      export_session()
    elif params['action'] == 'import_session':
      import_session()
